...
const CustomersData = () => {
  ...
  const validate = () => {
    ...
    if (!formData.cardEnding) errs.cardEnding = "Card Ending is required";
    if (!formData.categories) errs.categories = "At least one category required";
    if (!formData.clickthroughPercentage || isNaN(formData.clickthroughPercentage))
      ...
      <div className="sm:px-3 py-2 flex flex-col sm:flex-row justify-between sm:space-y-0 space-y-3">
        ...
        <div className="flex sm:w-auto sm:space-x-4 space-x-1 sm:pl-0 pl-3 w-full justify-between">
          ...
          <div className="flex justify-center items-center space-x-1 w-auto border border-gray-50 bg-blue-500 text-white rounded-3xl px-4 py-1">
            <button className="text-xl font-medium cursor-pointer" onClick={handleAddCustomer}>Add New</button>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white p-6 rounded-2xl w-96 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full border px-3 py-2 rounded-lg text-sm"
                value={formData.cardEnding}
                onChange={(e) => setFormData({ ...formData, cardEnding: e.target.value })}
              />
              {errors.cardEnding && <p className="text-red-500 text-sm">{errors.cardEnding}</p>}
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, clickthroughPercentage: e.target.value })}
              />
              {errors.clickthroughPercentage && <p className="text-red-500 text-sm">{errors.clickthroughPercentage}</p>}
            </div>
